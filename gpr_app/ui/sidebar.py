    ...
    },
    {
        ...
        "params": [
            {"key": "f_gain_mode", "widget": lambda *a, **kw: st.sidebar.selectbox(*a, **kw),
             "label": "Modalità Gain", "args": (["exponential", "curve"],),
    ...
    },
    {
        ...
        "params": [
            {"key": "f_hilbert_mode", "widget": lambda *a, **kw: st.sidebar.selectbox(*a, **kw),
             "label": "Modalità Hilbert", "args": (["envelope", "real", "imag", "phase"],), "kwargs": {}},
        ],
    },
    {
...
def build_filter_config() -> FilterConfig:
    """Costruisce la sidebar dei filtri con gerarchia UI migliorata."""
    ...
    # Expander per spiegazione
    with st.sidebar.expander("ℹ️ Come funziona", expanded=False):
        st.markdown(
            "**Workflow Base**: ordine fisso ottimizzato\n\n"
            "**Workflow Manuale**: controllo completo dell'ordine\n\n"
            "I filtri essenziali sono sempre visibili, quelli avanzati sono in expander separati."
        )
    
    orders_config: dict[str, dict[str, Any]] = {}
    
    # ESSENTIAL FILTERS (always visible)
    st.sidebar.markdown("### 🔧 Filtri Essenziali")
    for f_def in FILTER_DEFINITIONS:
        if f_def.get("category") != "essential":
            continue
        name = f_def["name"]
        enabled = st.sidebar.checkbox(f"Abilita {f_def['label']}", key=f"f_enable_{name}")
        order_manual = st.sidebar.number_input(
            f"Ordine {f_def['label']}",
    ...
    with st.sidebar.expander("🔬 Filtri Avanzati", expanded=False):
        for f_def in FILTER_DEFINITIONS:
            if f_def.get("category") != "advanced":
                continue
            name = f_def["name"]
            enabled = st.checkbox(f"Abilita {f_def['label']}", key=f"f_enable_{name}")
            order_manual = st.number_input(
                f"Ordine {f_def['label']}",
                min_value=0, max_value=20, step=1,
                key=f_def["key_order"],
                disabled=(not manual_mode) or (not enabled),
...
def build_visual_config() -> VisualConfig:
    ...
    with st.sidebar.expander("🎨 Colori e Visualizzazione", expanded=False):
        profile_raw_scale = st.selectbox("Scala colore radargramma originale", options=COLOR_SCALES, index=0)
        profile_filtered_scale = st.selectbox("Scala colore radargramma filtrato", options=COLOR_SCALES, index=1)
        profile_reverse = st.checkbox("Inverti colori radargrammi", value=False)
        timeslice_scale = st.selectbox("Scala colore time-slice", options=COLOR_SCALES, index=1)
        timeslice_reverse = st.checkbox("Inverti colori time-slice", value=False)
        points_scale = st.selectbox("Scala colore punti campionati", options=COLOR_SCALES, index=0)
        points_reverse = st.checkbox("Inverti colori punti campionati", value=False)
...
def build_decode_config() -> DecodeConfig:
    ...
    with st.sidebar.expander("🔍 Decoding OGPR", expanded=False):
...
def build_coordinate_config() -> CoordinateConfig:
    ...
    with st.sidebar.expander("📍 Coordinate GPS", expanded=False):
